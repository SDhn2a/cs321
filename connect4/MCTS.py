...
class Node(object):
    ...
    def updateValue(self, outcome):
        ...
        if math.isnan(self.value):
            self.value = 0
        self.visits += 1
        self.value = ((self.value*(self.visits-1))+outcome)/self.visits

    def UCBValue(self):
        """Value from the UCB formula used by parent to select a child. """
        "*** YOUR CODE HERE ***"
        util = (self.getValue()*self.state.getTurn()+1)/2
        if self.parent.visits == 0 or self.visits == 0:
            exploration = 1
        else:
            scaled_parent_visits = math.log(self.parent.visits)
            ratio = scaled_parent_visits / self.visits
            exploration = UCB_CONST*((ratio)**(1/2))
        return util + exploration
...
def MCTS(root, rollouts):
    ...
    for i in range(0, rollouts):
        find_eval_unvisited_child(root)
    best = -1
    best_option = -1
    for move in root.state.getMoves():
        child = root.children.get(move)
        if child:
            child_score = child.getValue()*root.state.getTurn()
            if child_score > best:
                best = child_score
                best_option = move
    return best_option
...
def find_eval_unvisited_child(root):
    if root.state.isTerminal():
        return root.state.value()
    for move in root.state.getMoves():
        if root.addMove(move):
            return play_game_randomly(Node(root.state.nextState(move), root))
    weights = []
    moves = []
    for move in root.state.getMoves():
        child = root.children.get(move)
        if child:
            child_util = child.UCBValue()
            weights.append(child_util)
            moves.append(move)
    weighted_random = random.choices(moves, weights)
    weighted_random_child = root.children.get(weighted_random[0])
    outcome = 0
    if weighted_random_child:
        outcome = find_eval_unvisited_child(weighted_random_child)
    root.updateValue(outcome)
    # print(root.getValue())
    return outcome


def play_game_randomly(start_node):
    if start_node.state.isTerminal():
        return start_node.state.value()
    else:
        new_move = random_move(start_node)
        new_state = start_node.state.nextState(new_move)
        return play_game_randomly(Node(new_state, start_node))
